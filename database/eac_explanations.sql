...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('What is the pH of a neutral solution at 25C?',
   'CHEM-101', 'At 25C, a neutral solution has equal [H] and [OH] concentrations, giving a pH of exactly 7.'),
  ('Balance this equation: Fe + O  FeO',
   'CHEM-101', 'The balanced equation is 4Fe + 3O  2FeO; balancing ensures equal numbers of each atom on both sides.'),
  ('Calculate the molar mass of HSO',
   'CHEM-101', 'HSO: (21) + 32 + (416) = 2 + 32 + 64 = 98 g/mol; molar mass is the sum of atomic masses of all atoms in the formula.')
) AS e(question_text, subject_code, explanation)
WHERE q.question_text = e.question_text
  AND s.code = e.subject_code
  AND s.id = q.subject_id_uuid
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('What is the binary equivalent of decimal 10?',
   'CS-101', '10 in decimal = 8+2 = 12 + 02 + 12 + 02 = 1010 in binary.'),
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('If sin  = 3/5, find cos ',
   'MATH-101', 'Using the identity sin + cos = 1: cos = 1  9/25 = 16/25, so cos  = 4/5.'),
  ('Solve for x: x - 5x + 6 = 0',
   'MATH-101', 'Factorising: (x  2)(x  3) = 0, giving x = 2 or x = 3; these are the two values that satisfy the equation.'),
  ('A circle has radius 7cm. What is its area? ( = 22/7)',
   'MATH-101', 'Area = r = (22/7)  7 = (22/7)  49 = 154 cm.')
) AS e(question_text, subject_code, explanation)
WHERE q.question_text = e.question_text
  AND s.code = e.subject_code
  AND s.id = q.subject_id_uuid
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('What is the refractive index of a medium if light travels at 210 m/s in it?',
   'PHY-101', 'Refractive index n = c/v = (310)/(210) = 1.5; it measures how much light slows down when entering a medium.'),
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('What type of reaction is: acid + base  salt + water?',
   'CHEM-101-NECO', 'This is a neutralisation reaction; an acid and a base react to form a salt and water, e.g. HCl + NaOH  NaCl + HO.'),
  ...
  ('What is the chemical formula of water?',
   'CHEM-101-NECO', 'Each water molecule has two hydrogen atoms covalently bonded to one oxygen atom, giving the formula HO.')
) AS e(question_text, subject_code, explanation)
WHERE q.question_text = e.question_text
  AND s.code = e.subject_code
  AND s.id = q.subject_id_uuid
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('What is the LCM of 12 and 18?',
   'MATH-101-NECO', '12 = 23, 18 = 23; LCM takes the highest power of each prime: 23 = 49 = 36.'),
...
FROM subjects s, exam_boards eb, (VALUES
  ('What is a covalent bond?',
   'CHEM-101-OLEVEL', 'A covalent bond forms when two non-metal atoms share one or more pairs of electrons to achieve a stable full outer shell, e.g. HO and CO.'),
  ('Describe the industrial production of ammonia (Haber Process)',
   'CHEM-101-OLEVEL', 'The Haber Process combines nitrogen (from air) and hydrogen (from natural gas) at 450C, 200 atm, using an iron catalyst: N + 3H  2NH; conditions are optimised for yield and rate.'),
  ...
  ('What gas is produced when copper reacts with concentrated HSO?',
   'CHEM-101-OLEVEL', 'Copper reacts with hot concentrated sulphuric acid: Cu + 2HSO  CuSO + SO + 2HO; the gas produced is sulphur dioxide (SO).'),
  ('What is the difference between an element and a compound?',
   'CHEM-101-OLEVEL', 'An element contains only one type of atom (e.g. O); a compound contains two or more different elements chemically bonded in fixed ratios (e.g. HO).')
) AS e(question_text, subject_code, explanation)
WHERE q.question_text = e.question_text
  AND s.code = e.subject_code
  AND s.id = q.subject_id_uuid
...
FROM subjects s, exam_boards eb, (VALUES
  ('Find the volume of a cylinder with radius 4cm and height 10cm',
   'MATH-101-OLEVEL', 'Volume = rh =   4  10 = 160  502.7 cm; the formula multiplies the circular cross-section area by the height.'),
  ('What is 15% of 200?',
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('Calculate the equivalent resistance of two 6 resistors in parallel',
   'PHY-101-OLEVEL', '1/R = 1/6 + 1/6 = 2/6 = 1/3, so R = 3; parallel resistors always give a combined resistance less than the smallest individual resistor.'),
...
FROM subjects s, exam_boards eb, (VALUES
  ...
  ('Calculate the pressure at the bottom of a 10m deep water column (density = 1000 kg/m)',
   'PHY-101-WAEC', 'Pressure = gh = 1000  10  10 = 100,000 Pa (100 kPa); pressure increases with depth due to the weight of fluid above.'),
  ...
  ('What is the unit of electrical resistance?',
   'PHY-101-WAEC', 'Electrical resistance is measured in Ohms (), defined by Ohm''s Law as the ratio of voltage to current: R = V/I.')
) AS e(question_text, subject_code, explanation)
WHERE q.question_text = e.question_text
  AND s.code = e.subject_code
  AND s.id = q.subject_id_uuid
