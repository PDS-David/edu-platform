...
export default function ContributeQuestion() {
  ...
  // Load exam boards
  useEffect(() => {
    axios.get(`${API}/exam-boards`, { headers: authHeader() })
      .then(r => setExamBoards(r.data.data || r.data))
      .catch(() => setExamBoards([]))
      .finally(() => setBoardsLoading(false));
  }, []);

  // Load subjects when board changes
  // FIX: exam board IDs are now UUIDs — compare as strings, not integers
  useEffect(() => {
    if (!form.exam_board_id) { setSubjects([]); return; }
    const board = examBoards.find(b => b.id === form.exam_board_id)?.code;
    if (!board) return;
    axios.get(`${API}/subjects?board=${board}`, { headers: authHeader() })
  ...
  // Submit
  // FIX: exam_board_id and subject_id are UUIDs — send as-is, no parseInt()
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await axios.post(`${API}/questions/submit`, payload, { headers: authHeader() });
      if (data.success) {
        setSubmitted(true);
      } else {
      ...
      alert(err?.response?.data?.error || 'Failed to submit question. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Contribute a Question</h1>
              <p className="text-xs text-gray-500">Help build the EAC question bank</p>
            </div>
          </div>
        </div>
