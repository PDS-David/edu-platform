...
async function runPreFlightValidation() {
  ...
  // NOTE: student_subjects.status, student_subjects.enrollment_source, and
  // student_exam_types.status do NOT exist yet at preflight time — they are
  // created by STEP 7. Pre-flight must NOT query them. Constraint-value checks
  // (status NOT IN (...) and enrollment_source NOT IN (...)) are performed
  // inside runVerificationSuite() which runs only after STEP 7 completes.
...
async function runDriftDetection() {
  ...
  // Expected schema shape BEFORE this migration runs.
  //
  // Columns created BY this migration must NOT appear here:
  //   • student_subjects.status           — added in STEP 7
  //   • student_subjects.enrollment_source — added in STEP 7
  //   • student_exam_types.status          — added in STEP 7
  //
  // Their absence at this point is the correct pre-migration state.
  // Listing them here would cause spurious DRIFT warnings on every clean run.
  // Post-migration existence is verified by runVerificationSuite() instead.
  const EXPECTED = {
    student_subjects: {
      columns: {
        // Only columns that must exist BEFORE the migration runs belong here.
        // added_at is verified separately in preflight (it may or may not exist —
        // STEP 1 handles both cases via ADD COLUMN IF NOT EXISTS).
      },
      constraints_must_not_exist: [], // pre-migration; we're about to add them
    },
    student_exam_types: {
      columns: {
        // No pre-existing columns asserted here; student_exam_types.status
        // is created by STEP 7, so it must not be listed as expected-before-migration.
      },
    },
  };
...
async function runVerificationSuite(mode) {
  ...
  // Constraint-value checks — these reference columns created by STEP 7 and
  // must therefore run here (post-migration), never in runPreFlightValidation().
  await verify('student_subjects: all status values are valid', async () => {
    const { rows } = await pool.query(`
  ...
  await verify('student_exam_types: all status values are valid', async () => {
    const { rows } = await pool.query(`
  ...
  await verify('student_subjects: all enrollment_source values are valid', async () => {
    const { rows } = await pool.query(`
      SELECT COUNT(*) AS n FROM student_subjects
