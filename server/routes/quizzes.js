...
router.post('/attempt', protect, async (req, res) => {
  ...
  try {
    ...
    for (const answer of answers) {
      ...
      // ASSIGN-marking-consistency fix (repo-wide audit): essay, structured,
      // and short_answer previously got inconsistent treatment across the
      // three practice surfaces — this file didn't AI-mark essay at all,
      // treated structured as an ungraded self-assessment excluded from
      // scoring, and gave short_answer no dedicated handling (it fell into
      // the MCQ branch below and scored wrong against any valid rephrasing
      // of correct_answer). Now all three route through the same AI
      // marking questionsRoutes.js and studentRoutes.js already use,
      // scaled to markValue, and DO count toward maxScore/totalScore like
      // every other question type — a graded question that's excluded from
      // the denominator was the original bug shape, not something worth
      // preserving now that real marking exists.
      const isAIMarked = question.type === 'essay' || question.type === 'structured' || question.type === 'short_answer';
      if (isAIMarked) {
        const answerText = (answer.essay_response ?? submittedAnswer ?? '').toString().trim();
        maxScore += markValue;
        ...
        if (process.env.GEMINI_API_KEY && answerText) {
          try {
            ...
            console.error('[POST /quizzes/attempt] AI marking failed:', aiErr.message);
            feedback = 'Submitted for manual review — automated marking was unavailable.';
          }
        } else if (!answerText) {
          feedback = 'No answer submitted.';
        } else {
          feedback = question.explanation || 'Compare your answer with the model answer above.';
        }
