...
router.get('/daily-study/:id', async (req, res) => {
  ...
  try {
    ...
    const rows = await sequelize.query(
      `SELECT DATE(completed_at) AS study_date, COALESCE(SUM(time_taken_seconds), 0)::INTEGER AS study_seconds
       FROM quiz_attempts
       WHERE student_id = :id AND status = 'completed' AND completed_at >= NOW() - INTERVAL '6 days'
       GROUP BY DATE(completed_at) ORDER BY study_date ASC`,
      { replacements: { id }, type: QueryTypes.SELECT }
    );
  ...
  try {
    ...
    const rows = await sequelize.query(
      `SELECT DATE_TRUNC('week', completed_at)::DATE AS week_start,
              COALESCE(ROUND(AVG(percentage)::NUMERIC, 2), 0) AS avg_accuracy
       FROM quiz_attempts
       WHERE student_id = :id AND status = 'completed' AND percentage IS NOT NULL
         AND completed_at >= NOW() - INTERVAL '48 days'
       GROUP BY DATE_TRUNC('week', completed_at) ORDER BY week_start ASC`,
      { replacements: { id }, type: QueryTypes.SELECT }
    );
